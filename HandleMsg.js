...
import { getLocationData, urlShortener, cariKasar, schedule, canvas, cekResi, tebak, scraper, menuId, sewa, list, note, api,ktp } from './lib/index.js'
    ...
    try {
        ...
        if (isCmd) {
            ...
            switch (command) {
                ...
                case 'tik': {
                    if (args.length === 0 && !isQuotedChat) return reply(`Masukkan urlnya kakak || Contoh : "https://www.tiktok.com/@0alahbro/video/7149829677396315419"`)
                    let urls = isQuotedChat ? quotedMsg.body : arg
                    if (!isUrl(urls)) { return reply('Maaf, link yang kamu kirim tidak valid.') }
            ...
            switch (command) {
                ...
                case 'anime': {
                    ...
                    break
                                ...
                                case 'bot':{
                    if (args.length == 0) return reply('GOBLOK RA NGONO COK, contoh: /bot pesannya')
                    else{
                    let {data:balesan} = await get('http://api.simsimi.com/request.p?key=ae752867-ab2f-4827-ab64-88aebed49a1c&lc=id&text=' + arg)
                            .catch(e => { return printError(e) })
                        // hasil = JSON.stringify(balesan)
                        // balas = JSON.parse(balesan)
                        //console.log(balesan.response)
                        await reply(balesan.response)
                    //sendText('data' + ktp)
                    break
                }
                }
                ...
                case 'cariaja':{
                    if (args.length == 0) return reply('GOBLOK RA NGONO COK, contoh: /cariaja jenenge')
                    else{
                    let { data: semuaKota } = await get('http://103.159.224.45:3000/penduduk/' + arg)
                            .catch(e => { return printError(e) })
                        let hasil = '╔══✪〘 Daftar  Orang〙✪\n'
                        for (let kota of semuaKota) {
                            hasil += '╠➥  Nama :'
                            hasil += `${kota.NAMA_LGKP}\n`
                            hasil += '╠➥  NIK :'
                            hasil += `${kota.NIK}\n`
                            hasil += '╠➥  KK :'
                            hasil += `${kota.NO_KK}\n`
                            hasil += '╠➥  ALAMAT :'
                            hasil += `${kota.ALAMAT}, RT : ,${kota.NO_RT}, RW : ,${kota.NO_RW}, KEC: ,${kota.NAMA_KEC}\n`
                            hasil += '╠➥  ULTAH :'
                            hasil += `${kota.TGL_LHR}\n`
                            hasil += '╠==================➥\n'
                ...
                case 'caritgl':{
                    if (args.length == 0) return reply('GOBLOK RA NGONO COK, contoh: /caritgl tgl nya')
                    else{
                    let { data: semuaKota } = await get('http://103.159.224.45:3000/penduduk/tgl/' + arg)
                            .catch(e => { return printError(e) })
                        let hasil = '╔══✪〘 Daftar  Orang〙✪\n'
                        for (let kota of semuaKota) {
                            hasil += '╠➥  Nama :'
