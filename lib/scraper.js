...
const ssstik = (browser, url) => new Promise(async (resolve, reject) => {
	try {
		...
		let mp4 = await page.$eval(
			"#target > div > div.result_overlay > a.pure-button.pure-button-primary.is-center.u-bl.dl-button.download_link.without_watermark.vignette_active",
			(element) => {
...
const snaptik = (browser, url) => new Promise(async (resolve, reject) => {
	try {
		...
		let d2 = await page.$eval(
			"div.snaptik-right > div > a:nth-child(1)",
			(element) => {
				return element.getAttribute("href")
			}
		)
